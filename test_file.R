...
my_data<-read.table("data_after_cleaning29.txt")
my_start_time<-"2022-02-5 22:30:10"
my_counts <- counts(data = my_data, hertz = 30,start_time = my_start_time,x_axis = 1,y_axis = 2,z_axis = 3)
x_minute<-c()
y_minute<-c()
z_minute<-c()
i<-1
...
#最后一个手动添加wake 状态
final_score<-append(final_score,2)
write(final_score,file = 'label29.txt')
